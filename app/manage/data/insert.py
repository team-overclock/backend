...
from ...models import Base, Region, Property, Infrastructure
from ...utils import parse_int, parse_float, run_with_progress, get_files

from ...parser.csv import read_csv_file
from ...parser.df import read_pandas, read_shp_file
...
def parse_address_data(_df: pd.DataFrame, *, silent: bool = False) -> pd.DataFrame:
    P = not silent
    if P:
        print()
    ...
    if P: print(f"  - ⏳ 2. pnu_code 컬럼 결합 연산 및 지번/도로명 주소 매핑 중...")
...
def parse_rnaddrkor(filepath: str, *_, silent: bool = False):
    P = not silent
    ...
    rdf = parse_address_data(rdf, silent=silent)
    if P: print(f"  - ⏳ 주소 변환을 위한 사전 매핑 중...")
    ADDRESS_MAP["rncode_to_rn"] = dict(zip(rdf["도로명코드"], rdf["도로명"]))
    DF["rnaddrkor"] = rdf

def parse_jibun_rnaddrkor(filepath: str, *_, silent: bool = False):
    jdf = read_pandas(
        filepath,
        names=[
            "도로명주소관리번호",
    ...
    jdf = parse_address_data(jdf, silent=silent)
    DF["jibun"] = jdf

def parse_AL_D164_shp(filepath: str, *_, silent: bool = False):
    P = not silent
    ...
    # ==========================================
    if P: print("  - ⏳ 1. 주거용 건물(단독/공동주택 등) 필터링 중...")
    ...
    # 2. 주소 매핑
    # ==========================================================
    if P: print(f"  - ⏳ 2. 주소 매핑용 룩업 사전 빌드 중...")
    ...
    if P: print("✨ [SUCCESS] 데이터 우선순위 역전 사전(ADDRESS_MAP) 빌드 완료!")

    # ==========================================
    # 3. 도로명주소관리번호 추출 및 주소 매핑용 컬럼 조립
    # ==========================================
    if P: print("  - ⏳ 3. 도로명주소관리번호 추출 및 주소 매핑용 컬럼 조립 중...")
    ...
    # ==========================================
    if P: print("  - ⏳ 4. 국토부 GDF에 대장 주소 매핑 중...")
    ...
    # ==========================================
    if P: print("  - ⏳ 5. 대장 미매칭 유령 필지 대상 국토부 내장 정보 결합 및 예외 처리...")
    ...
    # ==========================================
    if P: print("  - ⏳ 6. 주소 분리 조립 및 필수/Nullable 데이터 최종 추출 중...")
    ...
    # ====================================================
    if P: print("  - ⏳ 같은 아파트 단지(동별 데이터)를 하나의 대표 단지 데이터로 압축 중...")
    agg_rules = {
        "lon": "mean",
        "lat": "mean",
...
def parse_region(row: dict[str, Any], field_mapping: dict[str, Any], context: dict[str, Any], *, silent: bool = False):
...
def post_parse_region(context: dict[str, Any], *, silent: bool = False):
...
def parse_apartment(row: dict[str, Any], field_mapping: dict[str, Any], context: dict[str, Any], *, silent: bool = False):
...
def parse_school(row: dict[str, Any], field_mapping: dict[str, Any], context: dict[str, Any], *, silent: bool = False):
    name = get_field_value(row, field_mapping["이름"])
    source_id = get_field_value(row, field_mapping["id"])
...
def parse_subway_station(row: dict[str, Any], field_mapping: dict[str, Any], context: dict[str, Any], *, silent: bool = False):
    name = get_field_value(row, field_mapping["이름"])
    source_id = get_field_value(row, field_mapping["id"])
...
def post_parse_subway_station(context: dict[str, Any], *, silent: bool = False):
...
def parse_large_hospital(row: dict[str, Any], field_mapping: dict[str, Any], context: dict[str, Any], *, silent: bool = False):
    name = get_field_value(row, field_mapping["이름"])
    source_id = get_field_value(row, field_mapping["id"])
...
def parse_large_supermarket(row: dict[str, Any], field_mapping: dict[str, Any], context: dict[str, Any], *, silent: bool = False):
    name = get_field_value(row, field_mapping["이름"])
    source_id = get_field_value(row, field_mapping["id"])
    coords = get_coords_from_row(row, field_mapping)
    if not coords:
...
def parse_park(row: dict[str, Any], field_mapping: dict[str, Any], context: dict[str, Any], *, silent: bool = False):
    name = get_field_value(row, field_mapping["이름"])
    source_id = get_field_value(row, field_mapping["id"])
...
def connect_relationships(contexts: dict[str, Any], *, silent: bool = False):
...
def _run(db: Session, target_files: list[str], *, silent: bool = False):
    P = not silent

    if P: print(f"데이터베이스에서 기존 데이터 조회 중...")
            ...
            for filepath, filename in filtered_files:
                ...
                if is_iterable:
                    def run(idx: int, row: dict[str, Any]):
                        result = callback(row, field_mapping, context, silent=silent)
                        if not result: return

                        source_id = get_field_value(row, field_mapping["id"])
                        context.setdefault("results", {})[source_id] = result
                    run_with_progress(rows, prefix, run, interval=PROGRESS_PRINT, silent=silent)
                else:
                    def run(idx: int, filepath: str):
                        callback(filepath, field_mapping, context, silent=silent)
                    run_with_progress([filepath], prefix, run, interval=PROGRESS_PRINT, silent=silent)
    if P: print(f"데이터 정리 중...")
    for category in contexts:
        if P: print(f"  - {category}")
        ...
        match category:
            ...
            case _:
                continue
        callback(context, silent=silent)

    if P: print(f"데이터 삽입 중...")
    for category in contexts:
        match category:
            case "동네":
                Model = Region
        ...
        run_with_progress(items, f"[{category}]", run, interval=PROGRESS_PRINT, silent=silent)

    if P: print(f"테이블 관계 설정 중...")
    connect_relationships(contexts, silent=silent)

    if P: print(f"데이터 soft delete 처리 중...")
    for model, curr_map in MAP_ROUTER.items():
        items = list(curr_map.values())
        def run(idx: int, ins):
            ins.deleted_at = ins.deleted_at or func.now()
        run_with_progress(items, f"[{model.__tablename__}]", run, interval=PROGRESS_PRINT, silent=silent)


def run(directory_path: str | None = None, *, silent: bool = False):
    """
