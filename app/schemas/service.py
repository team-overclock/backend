...
class RecommendationReport(RecommendationCreateResponse):
    ...
    items: list[RecommendationReportItem] | None = Field(description="추천 매물 목록 (`status`가 `in_progress`인 경우에만 `null`)")


class UserRecommendation(RecommendationCreateResponse):
    """추천 요청 시 선택한 조건"""
...
class UserRecommendations(BaseModel):
    ...
    items: list[UserRecommendation] = Field(description="추천 요청 목록")
